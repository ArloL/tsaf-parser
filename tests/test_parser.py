...
def test_extract_apple_music_id_luvmaschine():
    data = _load("luvmaschine-localMediaItemLocations.bin")
    assert extract_apple_music_id(data) == 16256298393022529679


def test_extract_apple_music_id_just():
    data = _load("just-localMediaItemLocations.bin")
    assert extract_apple_music_id(data) == 14110461239496945263
...
def test_extract_title_luvmaschine():
    data = _load("luvmaschine-localMediaItemLocations.bin")
    assert extract_title(data) == "Luvmaschine Original Mix"


def test_extract_title_just():
    data = _load("just-localMediaItemLocations.bin")
    assert extract_title(data) == "Just (Original Mix)"
...
def test_extract_artist_luvmaschine():
    data = _load("luvmaschine-localMediaItemLocations.bin")
    assert extract_artist(data) == "Luvless"


def test_extract_artist_just():
    data = _load("just-localMediaItemLocations.bin")
    assert extract_artist(data) == "Bicep"
...
def test_extract_duration_luvmaschine():
    data = _load("luvmaschine-localMediaItemLocations.bin")
    assert extract_duration(data) == pytest.approx(414.8, abs=0.1)


def test_extract_duration_just():
    data = _load("just-localMediaItemLocations.bin")
    assert extract_duration(data) == pytest.approx(372.6, abs=0.1)
...
def test_extract_bpm_luvmaschine():
    data = _load("luvmaschine-mediaItemAnalyzedData.bin")
    assert extract_bpm(data) == pytest.approx(114.0, abs=0.1)


def test_extract_bpm_just():
    data = _load("just-mediaItemAnalyzedData.bin")
    assert extract_bpm(data) == pytest.approx(120.0, abs=0.1)


def test_extract_key_signature_index_luvmaschine():
    data = _load("luvmaschine-mediaItemAnalyzedData.bin")
    assert extract_key_signature_index(data) == 3


def test_extract_key_signature_index_just():
    data = _load("just-mediaItemAnalyzedData.bin")
    assert extract_key_signature_index(data) == 11
